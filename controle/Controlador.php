...
switch ($valorPagina) {
        ...
        if(!empty($_POST)){
            ...
            if(!empty($_FILES)){
                $conecFtp = new ConexaoFtp($this->ftpHost,
                        ...
                        $this->ftpSenha);
                $conecFtp->conectar(); 
                $caminhoFTP = "www/clientesx/gogo_MVC/visao/albuns/".$_POST['titulo'];
    ...
    case 34:
        ...
        if($_POST["album"]!="selecionar"){
            ...
            if(!empty($_FILES)){
                $caminhoFTP = "www/clientesx/gogo_MVC/visao/albuns/".$_POST['album'];               
               $upload = new UploadFTP($conecFtp->conecFTP,
                       $_FILES['file'],
    ...
    case 38:
        ...
        //deletando musicas dentro do zip
        $zip = new ZipArchive();

        if( $zip->open("albuns/".$_POST['album'].".zip")  == true){
            for($index=0;$index<$zip->numFiles;$index++){
                $temp = $zip->getNameIndex($index);
                $zip->deleteName($temp);
    ...
    case 47:
       ...
       $conecFtp = new ConexaoFtp($this->ftpHost,
        ...
        $caminhoFTP = "www/clientesx/gogo_MVC/visao/livros/";
        
        $uploadCapa = new UploadFTP($conecFtp->conecFTP,
                $_FILES['capa'],
                utf8_decode($_FILES["capa"]["name"]), 
                $caminhoFTP);
        
        $uploadLivro = new UploadFTP($conecFtp->conecFTP,
                $_FILES['livro'],
                utf8_decode($_FILES["livro"]["name"]), 
                $caminhoFTP);
